  ...
  return (
    <>
      <Head>
        ...
        <title>
          Mano | Un service gratuit dédié aux professionnels de maraude et de
          lieux d’accueil.
        </title>
      </Head>
      <header className="fixed z-20 flex items-center justify-between w-full px-4 py-5 bg-white border-b md:px-6 border-shamrock-50">
        ...
        >
          ...
          <MenuLink
            name="La protection des données"
            target="/#la-protection-des-donnees"
          />
          <MenuLink name="Qui sommes nous ?" target="/#qui-sommes-nous" />
          <MenuLink
            name="Ils nous font confiance"
            target="/#ils-nous-font-confiance"
          />
  ...
  <Link href={target}>
    <a
      className={`w-full p-4 text-sm font-medium text-black transition-all opacity-50 lg:w-auto lg:text-xs lg:p-0 hover:opacity-100 ${
        full &&
        "hidden lg:block rounded-lg lg:px-4 lg:py-2 text-white br-4 bg-shamrock-500 "
      }`}
    >
      {name}
    </a>
