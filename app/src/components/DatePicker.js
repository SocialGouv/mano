...
const DateAndTimeInput = ({ withTime, setWithTime, label, date, setDate, showTime }) => {
    ...
    return (
      <>
        ...
        <InputSubContainer onPress={onTimeChooseRequest}>
          ...
          <ButtonReset onPress={unsetWithTimeRequest} />
        </InputSubContainer>
      </>
    );
  };
  ...
  return (
    <>
      <InputContainer>
        ...
        <Inputs>
          <InputSubContainer onPress={onDateChooseRequest}>
            <Input asPlaceholder={!date.length}>{date ? convertDate(date) : 'JJ-MM-AAAA'}</Input>
            {Boolean(date.length) && <ButtonReset onPress={onClearDate} />}
          </InputSubContainer>
          {renderTime()}
        </Inputs>
      </InputContainer>
      <DatePicker
        mode={mode}
        visible={visible}
        initDate={date.length ? new Date(date) : new Date()}
        onClose={onClose}
        selectDate={setDateRequest}
      />
    </>
  );
};
