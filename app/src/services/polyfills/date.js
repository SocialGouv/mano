...
Date.prototype.getAge = function (locale) {
  const now = new Date();
  let years = now.getFullYear() - this.getFullYear();
  let suffix = locale === 'fr' ? `an${years > 1 ? 's' : ''}` : 'yo';
  if (now.getMonth() > this.getMonth()) return `${years} ${suffix}`;
  if (now.getMonth() < this.getMonth()) return `${years - 1} ${suffix}`;
  if (now.getDate() < this.getDate()) return `${years - 1} ${suffix}`;
  return `${years} ${suffix}`;
};
