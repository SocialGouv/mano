...
test("Cross teams report", async ({ page }) => {
  ...
  */
  ...
  await test.step("should be able to add a collab and a description in team 1", async () => {
    ...
    await page.getByRole("dialog").getByLabel("Description").fill(team1Description);
  ...
  await test.step("should be able to add a collab and a description in team 2", async () => {
    ...
    await page.getByRole("dialog").getByLabel("Description").fill(team2Description);
    await page.getByRole("button", { name: "Enregistrer" }).click();
  });
  ...
  await test.step("should see list of reports and being able to click on it, click on prev and next", async () => {
    ...
    await expect(page.getByText(`${dayjs().format("dddd D MMMM YYYY")}`).first()).toBeVisible();

    await page.getByRole("button", { name: "Précédent" }).click();

    await expect(page.getByText(`${dayjs().add(-1, "day").format("dddd D MMMM YYYY")}`).first()).toBeVisible();
