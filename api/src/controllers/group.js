...
router.post(
  ...
  catchErrors(async (req, res, next) => {
    try {
      ...
      const error = new Error(`Invalid request in group creation: ${e}`);
      error.status = 400;
      return next(error);
    }
...
router.get(
  ...
  catchErrors(async (req, res, next) => {
    try {
      ...
      const error = new Error(`Invalid request in group get: ${e}`);
      error.status = 400;
      return next(error);
    }
    ...
    const total = await Group.count(query);
...
router.put(
  ...
  catchErrors(async (req, res, next) => {
    try {
      ...
      const error = new Error(`Invalid request in group put: ${e}`);
      error.status = 400;
      return next(error);
    }
...
router.delete(
  ...
  catchErrors(async (req, res, next) => {
    try {
      ...
      const error = new Error(`Invalid request in group delete: ${e}`);
      error.status = 400;
      return next(error);
    }
    const query = { where: { _id: req.params._id, organisation: req.user.organisation } };
